const errorsPS = {
    addBoard: "Не удалось создать доску",
    getBoardData: "Не удалось получить данные доски",
    updateBoardData: "Не удалось обновить данные доски",
    getBoardsList: "Не удалось получить список досок",
    getUserName: "Не удалось получить имя пользователя",
    renameBoard: "Не удалось переименовать доску",
    downloadBoard: "Не удалось скачать доску",
    dBoard: "Не удалось загрузить доску"
};

export default errorsPS;